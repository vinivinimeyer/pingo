...
export function BottomNav() {
  ...
  const navItems = [
    ...
    { href: '/buscar', label: 'Buscar', icon: Search, key: 'buscar' },
    { href: null, label: 'Criar', icon: PlusCircle, key: 'criar', isCentral: true },
    { href: '/feed', label: 'Feed', icon: Rss, key: 'feed' },
    { href: '/perfil', label: 'Perfil', icon: User, key: 'perfil' },
  ];

  const isActive = (href: string | null, key: string) => {
            ...
            if (item.isCentral) {
              return (
                <button
                  ...
                  className="-mt-6 h-14 w-14 rounded-full bg-primary flex items-center justify-center shadow-lg transition-transform hover:scale-105 active:scale-95 hover:bg-primary/90"
